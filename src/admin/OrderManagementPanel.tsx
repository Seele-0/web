...
export function OrderManagementPanel({ orderDate, menu, api, onChanged }: {
  ...
  async function load() {
    ...
    setFeedback(null);
    try {
      updateOrder(await api.historyDetail(managedDate));
  ...
  async function addItem() {
    ...
    const ok = await mutate("add", () => api.adminUpsertOrderItem({ orderDate: managedDate, name: name.trim(), priceCents, quantity: parsedQuantity }), "已添加已点菜品");
    if (ok) {
  ...
  async function saveShareCount() {
    ...
    await mutate("share-count", () => api.adminSetOrderShareCount(managedDate, shareCount), "均摊人数已保存");
  }
  ...
  return <section className="admin-card" aria-labelledby="managed-order-title">
    ...
    {order && <>
      ...
      {activeTab === "import" && <div id="order-panel-import" role="tabpanel" aria-labelledby="order-tab-import" className="admin-tab-panel"><OrderImportPanel orderDate={managedDate} locked={locked} onImport={api.adminImportOrder} onImported={async (snapshot) => { updateOrder(snapshot); await notifyChanged(); }} /></div>}
