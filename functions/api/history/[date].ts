...
export const onRequestGet: PagesFunction<Env> = async ({ env, params, request }) => {
  try {
    ...
    const mealPeriod = new URL(request.url).searchParams.get("mealPeriod") ?? "dinner";
    ...
    const exists = await env.DB.prepare("SELECT order_date FROM daily_orders WHERE order_date = ?").bind(storageId).first();
