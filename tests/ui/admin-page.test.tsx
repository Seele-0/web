...
it("imports, deletes, corrects contributions, and clears a historical order from secondary tabs", async () => {
  ...
  await openOrderTab("贡献修正"); const quantity = screen.getByRole("spinbutton", { name: "张三的酸菜鱼数量" }); await userEvent.clear(quantity); await userEvent.type(quantity, "3"); await userEvent.click(screen.getByRole("button", { name: "保存张三的酸菜鱼数量" })); expect(api.adminCorrectContribution).toHaveBeenCalledWith({ orderDate: "2026-07-29", menuItemId: "dish-suan-cai-yu", deviceId: "device-a", displayName: "张三", quantity: 3 });
  await openOrderTab("菜品维护"); await userEvent.click(screen.getByRole("button", { name: "删除酸菜鱼" })); await userEvent.click(screen.getByRole("button", { name: "确认删除酸菜鱼" })); expect(api.adminDeleteOrderItem).toHaveBeenCalledWith("2026-07-29", "dish-suan-cai-yu");
  await openOrderTab("清空订单"); const clear = screen.getByRole("button", { name: "确认清空所选订单" }); expect(clear).toBeDisabled(); await userEvent.type(screen.getByLabelText("清空订单确认短语"), "清空订单 2026-07-29"); await userEvent.click(clear); expect(api.adminClearOrder).toHaveBeenCalledWith("2026-07-29");
});
